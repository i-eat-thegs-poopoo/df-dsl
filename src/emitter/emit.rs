...
use super::{ Program, Action, codeblock::{ Item, Block } };
...
impl <'a> EmitStmt<'a> {
    ...
    pub fn new(parent: &'a mut Program, name: String) -> Self {
        Self {
            parent,
            map: HashMap::new(),
            name,
    ...
    pub fn finalize(&mut self, jump: Option<String>, void: bool, last: bool) {
        let mut vec = mtake(&mut self.out);
        if void {
            vec.push(Block::Call("%var(%var(?d)?jump)".to_string()));
        }
        if last {
            vec.push(Block::Ctrl("End".to_string()));
        }
        self.map.insert(mtake(&mut self.name), vec);
        if let Some(v) = jump {
            self.name = v;
        };
    ...
    fn emit_if(
        ...
        macro_rules! if_body {
            (@if) => {
                ...
                if self.name != name {
                    self.map.insert(mtake(&mut self.name), mtake(&mut self.out));
                    self.out = self.map.remove(&name).expect("previous template missing after call");
                    self.name = name;
                }
            ...
            E::Func(span, e, args) => if let E::Ident(func) = *e {
                ...
                if let Some(v) = self.parent.args.get(&func) {
                    ...
                    self.out.append(&mut vec![
                        ...
                        code!(BlockAction, "="; Item::Var("%var(?d)?jump".to_string()), Item::Text(jump.clone()))
                    ]);
                    ...
                    self.finalize(Some(jump), false, false);
